...
function renderStatusBadge(isActive, activeText = 'Active', inactiveText = 'Inactive') {
  const statusClass = isActive ? 'status-active' : 'status-inactive';
  return `<span class="status-badge ${statusClass}">${isActive ? activeText : inactiveText}</span>`;
}

// Pagination UI updater
function updatePaginationUI(prefix, currentPage, total, pageSize, hasMore) {
  const totalPages = Math.ceil(total / pageSize) || 1;
  document.getElementById(`${prefix}-page-info`).textContent = `Page ${currentPage + 1} of ${totalPages}`;
  document.getElementById(`prev-${prefix}-page`).disabled = currentPage === 0;
  document.getElementById(`next-${prefix}-page`).disabled = !hasMore;
}

// Empty table row renderer
function renderEmptyRow(colspan, message) {
  return `<tr><td colspan="${colspan}" style="text-align: center; color: #636e72;">${message}</td></tr>`;
}

// Generic delete confirmation handler
async function confirmDelete(type, id, name, endpoint, onSuccess) {
...
// Populate select dropdown helper
function populateSelect(selectId, items, labelKey = 'name', valueKey = '_id', defaultOption = null) {
...
function showSection(section) {
  ...
  if (section === 'users') { loadUsers(); populateRoleDropdowns(); }
...
async function loadUsers(search = '') {
  try {
    ...
    if (users.length === 0) {
      tbody.innerHTML = renderEmptyRow(6, 'No users found.');
    } else {
    ...
    updatePaginationUI('', currentPage, total, pageSize, hasMore);
  } catch (error) {
...
async function deleteUser(userId, userName) {
  await confirmDelete('user', userId, userName, '/users', loadUsers);
}
...
async function loadRoles() {
  try {
    ...
    if (rolesCache.length === 0) {
      tbody.innerHTML = renderEmptyRow(6, 'No roles created yet. Click "Create New Role" to add one.');
      return;
    }
...
async function deleteRole(roleId, roleName) {
  await confirmDelete('role', roleId, roleName, '/roles', () => {
    loadRoles();
    loadRolesCache();
  });
}
...
function renderOptionsTable() {
  const typeMap = {
    ...
    'wb_loading_point': 'wbLoadingPoints',
  ...
  if (options.length === 0) {
    tbody.innerHTML = renderEmptyRow(5, 'No options found. Click "Add New Option" to create one.');
    return;
  }
  ...
  tbody.innerHTML = options.map(option => `
    <tr>
      ...
      <td>${option.name}</td>
      <td>${option.code || '-'}</td>
      <td>${renderStatusBadge(option.isActive)}</td>
      <td>
        <div class="action-buttons">
...
function getTypeDisplayName(type) {
  const names = {
    ...
    'wb_loading_point': 'WB Loading Point',
...
async function deleteOption(optionId, optionName) {
  await confirmDelete('option', optionId, optionName, '/dropdown-options', loadDropdownOptions);
}
...
async function loadTrips() {
  try {
    ...
    if (trips.length === 0) {
      tbody.innerHTML = renderEmptyRow(6, 'No trips found.');
    } else {
    ...
    updatePaginationUI('trips', tripsPage, total, tripsPageSize, hasMore);
  } catch (error) {
...
async function loadSuspiciousEntries() {
  try {
    ...
    if (entries.length === 0) {
      tbody.innerHTML = renderEmptyRow(6, 'No suspicious entries found.');
    } else {
    ...
    updatePaginationUI('suspicious', suspiciousPage, total, suspiciousPageSize, hasMore);
  } catch (error) {
