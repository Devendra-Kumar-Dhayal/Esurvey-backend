...
/**
 * Get WB loading points for dropdown (Way Bridge specific)
 * GET /api/qr/wb-loading-points
 */
const getWBLoadingPoints = async (req, res) => {
  try {
    const wbLoadingPoints = await DropdownOption.find({
      type: 'wb_loading_point',
      isActive: true
    }).sort({ order: 1, name: 1 });

    sendSuccess(res, { wbLoadingPoints }, 'WB loading points retrieved');
  } catch (error) {
    console.error('Get WB loading points error:', error);
    sendError(res, 'Failed to get WB loading points', 500);
  }
};
...
const saveWayBridgeData = async (req, res) => {
  try {
    ...
    // Validate WB loading point (Way Bridge uses wb_loading_point type)
    ...
    if (!loadingPoint) {
      return sendError(res, 'Invalid WB loading point', 400);
    }
